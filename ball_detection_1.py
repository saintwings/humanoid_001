...
class ObjectDetection_2():
  ...
  def ball_tracking(self):
    ...
    interpreter = self.make_interpreter(self.MODEL)
    ...
    while True:
      ...
      if image.size != (self.width, self.height):
          img = imutils.resize(img, height=self.height, width=self.width)
          image = image.resize((self.width,self. height))
      scale = detect.set_input(interpreter, (self.width, self.height), lambda size: image.resize(size, Image.ANTIALIAS))
      ...
      # cv2.imshow(WINDOW_NAME, img)
          cv2.imshow(self.WINDOW_NAME, cv2.cvtColor(np.asarray(image), cv2.COLOR_RGB2BGR))
          key = cv2.waitKey(1)
          if key == ord('q'):
            break
          elif key == ord('f'):
            full_screen = not full_screen
          if self.full_screen: cv2.setWindowProperty(WINDOW_NAME, cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)
          else: cv2.setWindowProperty(self.WINDOW_NAME, cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_NORMAL)
