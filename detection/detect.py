...
class BBox(collections.namedtuple('BBox', ['xmin', 'ymin', 'xmax', 'ymax'])):
  ...
  def fit(self, width, height):# Avoid BBox out of range
    dx, dy = 0, 0
    if self.xmin < 0: dx = abs(self.xmin)
    if self.ymin < 0: dy = abs(self.ymin)
    if self.xmax >= width: dx = width - self.xmax - 1
    if self.ymax >= height: dy = height - self.ymax - 1
